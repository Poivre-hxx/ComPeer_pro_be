...
def load_email_receivers(file_path):
    email_receiver_list = {}
    
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            for line in file:
                # 每一行格式应为 'userID: user_email'
                line = line.strip()
                if line:
                    user_id, email = line.split(":")
                    email_receiver_list[user_id.strip()] = email.strip()
        print(f"Successfully loaded {len(email_receiver_list)} email addresses.")
...
if __name__ == '__main__':
    ...
    email_receiver_List = load_email_receivers("./user_email.txt")
    initial_reflection = {}
    for user_id in user_ids:
        user_states[user_id] = {"last_check": time.time(), "last_length": None, "messages": []}
        initial_reflection[user_id]=""
        safety_monitor = send_email.SafetyMonitor(email_sender, email_password, email_receiver_List,user_id)
